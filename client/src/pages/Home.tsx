...
const pieces = [
  { number: "01", title: "HoodWink Tee", detail: "* 1x1 RIB KNIT COLLAR *\n* 100% COTTON *\n* CREW-NECK *\n* 200GSM", price: "$128", image: "/manus-storage/hoodwink-tee_73f0ebf4.webp", note: "One side says stay hidden. The other says you were never hiding." },
  { number: "02", title: "EXPTL. Band Tee", detail: "Two collars", price: "$214", image: "/manus-storage/exptl-band-tee_487daf4d.webp", note: "A familiar silhouette with an unfamiliar point of view." },
  { number: "03", title: "Muscle Memory Tee", detail: "Hidden pocket", price: "$146", image: "/manus-storage/muscle-memory-tee_69a48a50.webp", note: "The detail you miss is the detail that matters most." },
];
  ...
  return (
    <main className={`site-shell ${editMode ? "is-editing" : ""}`} onPointerMove={(event) => { moveDrag(event); movePanel(event); movePan(event); }} onPointerUp={() => { endDrag(); setPanelDragging(false); setIsPanning(false); }}>
      <header className="nav-wrap">
        ...
        <div className="nav-actions"><a className="nav-mark" href="#manifesto" aria-label="Jump to In Plain Sight">↗</a></div>
      </header>
      ...
      <section className="hero" id="top" onClick={(event) => { if (editMode && event.target === event.currentTarget) setSelectedId(null); }}>
        <div className="hero-portal-field" aria-hidden="true"><Suspense fallback={<div className="portal-fallback" />}><LazyPortalField mode="dark" speed={0.16} size={0.82} length={0.72} density={0.25} opacity={0.48} hue={145} saturation={0.78} brightness={0.62} /></Suspense></div>
