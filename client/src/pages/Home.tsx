...
export default function Home() {
  ...
  return (
    <main className={`site-shell ${editMode ? "is-editing" : ""}`} onPointerMove={(event) => { moveDrag(event); movePanel(event); movePan(event); }} onPointerUp={() => { endDrag(); setPanelDragging(false); setIsPanning(false); }}>
      ...
      <footer className="footer"><div className="footer-brand"><Editable id="footer-mark" label="Footer mark" editMode={editMode} selectedId={selectedId} onSelect={setSelectedId} onPointerDown={startDrag}>HOODWINK / 2026</Editable><Editable id="footer-note" label="Footer note" editMode={editMode} selectedId={selectedId} onSelect={setSelectedId} onPointerDown={startDrag}>A trick, worn well.</Editable><a className="footer-back" href="#top"><Editable id="footer-back" label="Footer back link" editMode={editMode} selectedId={selectedId} onSelect={setSelectedId} onPointerDown={startDrag}>Back to top ↑</Editable></a></div><div className="footer-aside"><span className="footer-social-label">Find us in plain sight</span><div className="social-links" aria-label="HoodWink social links">{socialLinks.map(({ label, href, icon: Icon }) => <a key={label} className="social-link" href={href} target="_blank" rel="noreferrer" aria-label={label} title={label}>{Icon ? <Icon size={15} strokeWidth={1.7} /> : <span className="x-glyph">𝕏</span>}</a>)}</div></div></footer>
      </div>
      </div>
    </main>
  );
